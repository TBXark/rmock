...
program
    .addOption(new Option('-p, --port <port>', 'port').argParser(parseInt).default(3000)) // eslint-disable-line max-len
    .option('-t, --target <url>', 'target url', 'https://api.github.com')
    .addOption(new Option('-tp, --targetPort <number>', 'target port').argParser(parseInt)) // eslint-disable-line max-len
    ...
    .addOption( new Option('-i, --importer <mode>', 'import external code mode').choices(['vm', 'cjs', 'esm']).default('vm')) // eslint-disable-line max-len
...
// 导入模式
export const IMPORTER_MODE = options.importer;
