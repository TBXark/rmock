...
program
    .addOption(new Option('-p, --port <port>', 'port', 3000).argParser(parseInt))
    .option('-t, --target <url>', 'target url', 'https://api.github.com')
    .addOption(new Option('-tp, --targetPort <number>', 'target port', null).argParser(parseInt))
    ...
    .addOption(new Option('-i, --importer', 'import external code mode: vm,esm,cjs', 'vm').choices(['vm', 'esm', 'cjs']))
