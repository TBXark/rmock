...
export function mapJSON(map, config) {
  return async (ctx) => {
    ...
    await mapResponse(requestBuilder, responseBodyGetter, responseStringify, config, map)(ctx)
  };
}

export function mapResponse(requestBuilder, responseBodyGetter, responseStringify, config, map) {
  return async (ctx) => {
    const {
      canLog,
      showRequestHeader,
