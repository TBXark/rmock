...
export function mapJSON(map, config) {
  return async (ctx) => {
    ...
    const responseBodyGetter = async (res) => {
      return await res.json();
    }
    await customMapper(requestBuilder, responseBodyGetter, responseStringify, config, map)(ctx)
  };
}

export function customMapper(requestBuilder, responseBodyGetter, responseStringify, config, map) {
  return async (ctx) => {
    const {
      canLog,
      showRequestHeader,
